# Google Business Profile Update

Use these details to align Google Business Profile, Google search snippets, and the website. Do not publish a phone number, physical address, or hours until those exact public details are confirmed.

## Core Profile

- Business name: N8iV Promotions
- Primary category: Marketing agency
- Secondary categories: Internet marketing service, Marketing consultant, Advertising agency
- Website: https://www.n8ivpromotions.com/
- Public contact email: zajen@n8ivpromotions.com
- Service area: Scottsdale, Phoenix, Arizona, and remote United States B2B and agency clients

## Business Description

N8iV Promotions is a revenue intelligence marketing agency helping B2B businesses and agencies connect SEO, social content, ad campaigns, CRM activity, and closed revenue. Powered by ARIE, the Automatic Revenue Intelligence Engine, N8iV provides attribution, SEO strategy, social content management, analytics reporting, and client-ready revenue intelligence reports so marketing decisions are backed by real data.

## Services

- Revenue Intelligence & Attribution
- SEO Strategy
- Social Media Content Management
- Marketing Analytics Reporting
- Agency Partner Analytics
- HubSpot Revenue Reporting
- Paid Media Attribution
- ARIE Revenue Intelligence Audit

## Suggested Search Phrases

- revenue intelligence marketing agency
- B2B marketing attribution agency
- marketing analytics agency Scottsdale
- SEO and analytics agency Phoenix
- social media content management and reporting
- HubSpot revenue reporting agency
- agency client attribution reporting

## Profile Assets To Upload

- Logo: `assets/logos/n8iv-promotions.svg`
- Cover image: `assets/dashboard-hero.png`
- Service visual: `assets/platform-hero.png`
- Audit visual: `assets/audit-hero.png`

## Items To Confirm Before Publishing

- Public phone number
- Physical address or service-area-only listing
- Business hours
- Appointment URL, if different from https://www.n8ivpromotions.com/contact.html
- Google Business Profile short name or profile URL
- Social profile URLs to include in future website schema
